...
class Writer(object):
    def __init__(self, path, width, height, depth=3, database='Unknown', segmented=0):
        loader = FileSystemLoader(searchpath="./resource")
        environment = Environment(loader=loader, keep_trailing_newline=True)
    ...
    def addFromObjs(self, objs):
        names, bboxes, difficults = objs['category_name'], objs['bboxes'], objs['difficult']
        self.addBboxes(bboxes, names, difficults)
